...
- **Dependencies:** Will you mock `boredAPI` and just assert that `getNewActivity` was called? Or will you use the original module and assert directly the `fetch` request (as we did in `1.6`)? Mocking the module is slightly easier. Asserting the fetch gives you extra confidence. Two options, your choice!
